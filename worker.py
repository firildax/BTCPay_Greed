...
class successfulPaymentBTC(threading.Thread):
    # constructor
    def __init__(self):
    ...
    def run(self, invoiceId,token,store):
        ...
        while self.has_invoice:
            res = requests.get(uri, headers=headers)
            invoice = json.loads(res.content)
            if invoice['status'] == 'New':
                # invoice payment successfull
                # return invoiceid
                # return storeid
                continue
...
class Worker(threading.Thread):
    ...
    def __wait_for_successfulpayment_btcpay(self, invoiceId,cancellable: bool = False) -> Union[telegram.SuccessfulPayment, CancelSignal]:
        ...
        while True:
            ...
            if invoice['status'] == 'New':
                ...
                payment = { 'status' : invoice['status'], 'invoiceId': invoice['id'], 'storeId': invoice['storeId'],'order_info': None }
                break
            elif invoice['status'] == 'Expired':
                # invoice payment failed
                # return invoice id
    ...
    def __add_credit_menu(self):
        ...
        # BTCPay Payments
        if self.cfg["Payments"]["BTCPay"]["token"] != "":
            keyboard.append([telegram.KeyboardButton(self.loc.get("menu_btcpay"))])
        ...
        # Wait for a reply from the user
        selection = self.__wait_for_specific_message(
            [self.loc.get("menu_cash"), self.loc.get("menu_btcpay"),self.loc.get("menu_credit_card"), self.loc.get("menu_cancel")],
            cancellable=True)
        ...
        # If the user has selected the BTCPay option...
        elif selection == self.loc.get("menu_btcpay") and self.cfg["Payments"]["BTCPay"][
            "token"]:
            # Go to the pay with BTCPay function
            self.__add_btcpay_cc()
    ...
    def __add_btcpay_cc(self):
        """Add money to the wallet through a btcpay payment."""
        log.debug("Displaying __add_btcpay_cc")
        # Create a keyboard to be sent later
        presets = self.cfg["Payments"]["BTCPay"]["payment_presets"]
        keyboard = [[telegram.KeyboardButton(str(self.Price(preset)))] for preset in presets]
        keyboard.append([telegram.KeyboardButton(self.loc.get("menu_cancel"))])
        # Boolean variable to check if the user has cancelled the action
        cancelled = False
        ...
        while not cancelled:
            ...
            value = self.Price(selection)
            # Ensure the amount is within the range
            if value > self.Price(self.cfg["Payments"]["BTCPay"]["max_amount"]):
                self.bot.send_message(self.chat.id,
                                      self.loc.get("error_payment_amount_over_max",
                                                   max_amount=self.Price(self.cfg["BTCPay"]["max_amount"])))
                continue
            elif value < self.Price(self.cfg["Payments"]["BTCPay"]["min_amount"]):
                self.bot.send_message(self.chat.id,
                                      self.loc.get("error_payment_amount_under_min",
                                                   min_amount=self.Price(self.cfg["BTCPay"]["min_amount"])))
                continue
            break
        # If the user cancelled the action...
        else:
            ...
            return
        # Issue the payment invoice
        self.__make_payment_btcpay(amount=value)
    ...
    def __make_payment_btcpay(self, amount):
        ...
        # Send the keyboard to the user
        self.bot.send_photo(self.chat.id, photo=open('./pay.png', 'rb'), caption= checkoutLink)

        # Wait for the payment
        # successfulpaymentbtc = threading.Thread(target=self.__wait_for_successfulpayment_btcpay(invoiceId))
        ...
        if x.status == 'Settled':
            ...
            transaction = db.Transaction(user=self.user,
                                     value=int(amount),
                                     provider="BTCPay",
                                     telegram_charge_id=x.storeId,
                                     provider_charge_id=x.invoiceId)

            if x.order_info is not None:
                transaction.payment_name = x.name
                transaction.payment_email = x.mail
                transaction.payment_phone = x.phone_number
